  ...
  return (<div className='relative'>
    ...
    <div className='w-full bg-white z-10 h-20 flex items-center justify-between md:px-10 px-4 drop-shadow-md fixed'>
      ...
      <div className='gap-2 flex items-center cursor-pointer' >
        <div onClick={handleHistory} className='rounded-full p-6 bg-[url("../assets/profile.jpg")] bg-cover'><p className=''></p></div>
        <p onClick={handleHistory} className='text-xl md:text-2xl text-black-100 '>{user.name}</p>
    ...
    <button className='hidden' onClick={() => console.log(state.redeemed)}>mostrar</button>
        {
          showHistory
          ? <History />
          : ""
